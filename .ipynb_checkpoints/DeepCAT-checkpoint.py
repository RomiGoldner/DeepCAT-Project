...
def TrainAndEvaluate(TrainFeature, TrainLabels, EvalFeature, EvalLabels, STEPs=10000, ID='', dir_prefix='/tmp/', my_model=False):
    ## Train CNN model:
    for LL in range(12,17): # TODO ROMI change to 17 here and bellow
        print("doing for CDR len", LL)
        if my_model:
            ...
            ModelDict_SVM[LL] = clf
            eval_results = {'accuracy': clf.score(x, y)}# , 'loss': 0.5233598, 'global_step': 20000}
        else:
...
def PredictEvaluation(EvalFeature,EvalLabels=None,makePlot=False,ID='',dir_prefix=curPath+'/tmp/', my_model=False):
    PredictClass={}
    PredictLabels={}
    AUCDict={}
    for LL in range(14,17):
        if my_model:
            clf = ModelDict_SVM[LL]
        else:
            CDR3_classifier=tf.estimator.Estimator(model_fn=ModelDict[LL],model_dir=dir_prefix+'/CDR3_classifier_PCA_LL'+str(LL)+'_L2_k2f8d10_'+ID+'/')
        if EvalLabels is None:
            if my_model:
                ...
                # Create list of dictionaries, each contains "classess" and "probabilities"
                eval_results = []
                for i in range(len(y_pred)):
                    eval_results.append({'classes': y_pred[i].argmax(), 
                                         'probabilities': y_pred[i].astype(np.float32)})
            else:
                eval_input_fn=tf.estimator.inputs.numpy_input_fn(
                    x={'x':EvalFeature[LL]['x']},        
                    num_epochs=1,
...
def batchTrain(ftumor, fnormal,feval_tumor,feval_normal, rate=0.33,n=100,STEPs=10000,dir_prefix=curPath+'/tmp', my_model=False):
    ...
    while count<n:
        ...
        TrainAndEvaluate(TrainFeature, TrainLabels, EvalFeature, EvalLabels,STEPs=STEPs,ID=ID,dir_prefix=dir_prefix, my_model=my_model)
        PC,PL,AD=PredictEvaluation(EvalFeature,EvalLabels=EvalLabels,makePlot=False,ID=ID,dir_prefix=dir_prefix, my_model=my_model)
